...
#include <cstddef>
//#define M_WIDTH (8 * sizeof(uint16_t)))
...
namespace Core
{
  namespace Utils
  {
    ...
    public:
      ...
      CRC16(uint8_t *message, int size)
      {
        uint16_t remainder = 0;
        return accumulatesCRC16(remainder, message, size);
      ...
      static inline uint16_t
      accumulatesCRC16(const uint16_t& crc, uint8_t byte)
      {
        uint16_t remainder=crc;
        return (remainder >> 8) ^ crc_table[(remainder ^ byte) & 0xff];
      }
      static inline uint16_t
      accumulatesCRC16(const uint16_t& crc, uint8_t *message, int size)
      {
        uint16_t remainder=crc;
        while (size--)
          remainder = (remainder >> 8)
              ^ crc_table[(remainder ^ *message++) & 0xff];
        return remainder;

 ...
 *
 */
class GatewayServer : public ROSNode
{
public:
  GatewayServer();
  virtual
...
protected:
  ...
  //!
  std::vector<RemoteClient_t> subscribers_;
  ...
  close();
  void
  handleClientConnection(GatewayClient& c, DATA_ARRAY& data);
  void
  handleNewConnection(RemoteClient_t * client);
  ...
  setupRemoteMasters();
  bool
  parse(uint8_t ch, RemoteMaster_t& m);
  void
  resetRemoteMaster(RemoteMaster_t& rm);
  void
  republishMsgToInternalNetwork(const RemoteMaster_t& m);
  void
...
private:
  ...
  setupSocketType();
  ssize_t
  readFromSocket(GatewayClient&  c, uint8_t * buf, size_t sz);
  void
  sendToSocket(const RemoteClient_t& c, size_t dsz, uint8_t* data);
};

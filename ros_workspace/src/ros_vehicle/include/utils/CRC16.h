...
{
  static uint16_t
  accumulate(const uint16_t& crc, uint8_t byte)
  {
    uint16_t remainder = crc;
    return (remainder >> 8) ^ crc_table[(remainder ^ byte) & 0xff];
  }
  static uint16_t
  accumulate(const uint16_t& crc, uint8_t *message, int size)
  {
    uint16_t remainder = crc;
    while (size--)
      remainder = (remainder >> 8) ^ crc_table[(remainder ^ *message++) & 0xff];
    return remainder;

  }
  static uint16_t
  check(uint8_t *message, int size)
  {
    uint16_t remainder = 0;
    return accumulate(remainder, message, size);
